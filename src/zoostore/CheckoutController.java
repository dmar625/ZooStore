...
import zoostore.model.Customer;
import zoostore.model.Product;
...
public class CheckoutController {
    ...
    protected Alert createInfoAlert() {
        ...
        alert.getDialogPane().setContentText("Możesz wyświetlić zakupione towary w zakładce Historia.");
        alert.getDialogPane().setHeaderText("Twój zakup zakończył się sukcesem.");
        alert.showAndWait()
    ...
    protected Alert createErrorAlert() {
        ...
        alert.initModality(Modality.APPLICATION_MODAL);
        alert.initOwner(stage);
        alert.getDialogPane().setContentText("Uzupełnij środki na koncie i spróbuj ponownie...");
        alert.getDialogPane().setHeaderText("Twój stan konta jest niewystarczający.");
        alert.showAndWait()
                .filter(response -> response == ButtonType.OK);
        return alert;
    }
    ...
    public void purchaseButtonClicked(ActionEvent event) throws SQLException, ParseException, IOException {
        ...
        if( total <= balance) {
            ...
            checkoutCartButton.setText("Koszyk");
            Main.getInstance().gotoZooStore();
        } else {
