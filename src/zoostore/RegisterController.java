...
import zoostore.model.CustomerQueries;
...
public class RegisterController implements Initializable {
    ...
    protected Alert createAlert() {
        ...
        alert.getDialogPane().setContentText(type + " Zarejestrowałeś się w Zoo Store. Możesz się teraz zalogować.");
        alert.getDialogPane().setHeaderText(null);
        alert.showAndWait()
                .filter(response -> response == ButtonType.OK)
                .ifPresent(response -> System.out.println("Alert zaakceptowany"));
        return alert;
    }
    ...
    public void initialize(URL url, ResourceBundle rb) {
        registerUserName.setPromptText("Wprowadź nazwę");
        registerPassword.setPromptText("Wprowadź hasło");
        registerEmail.setPromptText("example@example.com");
        registerCreditCard.setPromptText("16 cyfr");
    }    
    ...
    public void processRegister(ActionEvent event) throws IOException {
        ...
        if(registerUserName.getText().trim().isEmpty() || registerPassword.getText().trim().isEmpty() || registerEmail.getText().trim().isEmpty() || registerCreditCard.getText().trim().isEmpty() || registerCreditCard.getText().length() != 16 || !(registerCreditCard.getText().matches("[0-9]+")) || !(registerEmail.getText().matches("\\b[a-z0-9._-]+@[a-z0-9.-]+\\.[a-z]{2,}\\b"))) {
            errorLabel.setText("Proszę wprowadzić dane zgodne z wymogami.");
        } else {
